  ...
  async function callLLM(messages, images) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (/image|vision|multimodal|not support|unsupported/i.test(t)) {
          toast('当前模型不支持看图，请换成免费视觉模型 glm-4.6v-flash（智谱）');
        } else {
          toast('大模型调用失败：' + res.status);
        }
        console.warn('LLM err', res.status, t); return null;
      }
...
我的需求：${prompt}${outfitAiImg ? '\n（附上了我的穿搭照片，请帮我看哪件配饰更搭这套）' : ''}`;
    ...
    resBox.innerHTML = `<div class="outfit-ai-result">${content ? esc(content).replace(/\n/g,'<br/>') : 'AI 未返回结果。<br/>① 检查「唱歌 → ⚙️ AI 设置」是否填了 Key；<br/>② 上传照片需要<b>视觉模型</b>，DeepSeek 看不了图。<br/>推荐<b>永久免费</b>的：智谱 glm-4.6v-flash（地址 https://open.bigmodel.cn/api/paas/v4）。'}</div>`;
  }
