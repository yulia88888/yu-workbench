    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (/image|vision|multimodal|not support|unsupported/i.test(t)) {
          toast('当前模型不支持看图，请在⚙️设置换成视觉模型（gpt-4o / 通义千问VL / 智谱 GLM-4V / 豆包视觉）');
        } else {
...
我的需求：${prompt}${outfitAiImg ? '\n（附上了我的穿搭照片，请帮我看哪件配饰更搭这套）' : ''}`;
    ...
    resBox.innerHTML = `<div class="outfit-ai-result">${content ? esc(content).replace(/\n/g,'<br/>') : 'AI 未返回结果，请检查「唱歌」模块里的 ⚙️ AI 设置是否已填写免费 Key；若上传了照片，请确认所用模型支持看图（如 gpt-4o / 通义千问VL / 智谱 GLM-4V / 豆包视觉）。'}</div>`;
  }
